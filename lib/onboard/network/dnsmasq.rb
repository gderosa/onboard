...
class OnBoard
  module Network
    class Dnsmasq
      ...
      def write_addn_hosts(h)
        # File starting with a '.' are not read as configuration files,
        # but may be read as additional /etc/hosts files.
        hosts_file = "#{CONFDIR_CURRENT}/.#{h[:table]}"
        hosts_conf = "#{CONFDIR_CURRENT}/#{h[:table]}.conf"
        File.open hosts_file, 'w' do |f|
          f.puts banner
          h[:data].each_pair do |addr, names|
            if names.respond_to? :join
              f.puts "#{addr} #{names.join ' '}"
            else
              f.puts "#{addr} #{names}"
            end
          end
        end
        File.open hosts_conf, 'w' do |f|
          f.puts banner
          f.puts "addn-hosts=#{hosts_file}"
        end
      end
