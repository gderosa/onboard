  ...
  module Network
    class Dnsmasq
      ...
      def write_domains_conf_from_HTTP_request(params)
        str = "# Automatically generated by #{self.class.name} \n\n"
        params['domain'].each_with_index do |domain, n|
          domain.strip!
          domain.downcase!
          next unless domain =~ /^[\w\-\.]+$/
          ip = params['ip'][n]
          if OnBoard::Network::Interface::IP::valid_address? ip
            str << "address=/#{domain}/#{ip}\n" 
          else
            str << "address=/#{domain}/0.0.0.0\n"
            str << "address=/#{domain}/::\n"
          end
        end
        str << "\n"
        ...
        dns_conf_file = "#{CONFDIR_CURRENT}/dns.conf"
